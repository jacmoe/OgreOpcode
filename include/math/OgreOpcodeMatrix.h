...
*/
#ifndef __OgreOpcodeMatrix_h__
#define __OgreOpcodeMatrix_h__

#include "math/OgreOpcodeReal.h"
#include "math/OgreOpcodeMinmax.h"
#include <cmath>
#include <limits>
#include <algorithm>
#include "math/OgreOpcodeVector3.h"
#include "math/OgreOpcodeVector4.h"
#include "math/OgreOpcodeEuler.h"
	...
	namespace math
	{
		...
		struct Matrix4x4
		{
			...
			*/
			...
			void LoadMatrix(const Vector3<T>& x, const Vector3<T>& y, const Vector3<T>& z)
			{
				m44[0][0] = x[0];
				m44[0][1] = x[1];
				m44[0][2] = x[2];
			...
			Matrix4x4<T>& GetRotationMatrix() const
			{
				...
				rot.m44[3][2] = 0.0;
				rot.m44[3][3] = 1.0;
				return rot; // FIXME: returning address of local variable
			}

			Vector3<T>& GetAxis(int i) const
			{
				return Vector3<T>(m44[0][i], m44[1][i], m44[2][i]);// FIXME: returning address of local variable
			}
			
			Vector3<T>& GetPosition() const
			{
				return Vector3<T>(m16[12], m16[13], m16[14]);// FIXME: returning address of local variable
			}
		...
		template <typename T>
		inline bool IsEqual(const Matrix4x4<T>& v, const Matrix4x4<T>& matrix, real diff = 0.001f)
		{
			for (int i = 0; i < 16; i++)
			{
				if (fabs(v.m16[i] - matrix.m16[i]) > diff)
				{
					return false;
				}
			}
		...
		typedef Matrix4x4<real>	Matrix_r;

	#endif // OGREOPCODEMATH_NOTYPENAME
	} // namespace math
} // namespace OgreOpcode

#endif // __OgreOpcodeMatrix_h__